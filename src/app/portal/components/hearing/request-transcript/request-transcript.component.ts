...
export class RequestTranscriptComponent implements OnInit, OnDestroy {
  ...
  onConfirm(moreDetail: string) {
    const transcriptionRequest: TranscriptionRequest = {
      ...
      comment: moreDetail,
      start_date_time: this.audioTimes?.startTime
        ? this.audioTimes?.startTime?.toUTC().toISO()?.split('.')[0] + 'Z'
        : '',
      end_date_time: this.audioTimes?.startTime ? this.audioTimes?.endTime?.toUTC().toISO()?.split('.')[0] + 'Z' : '',
    };
